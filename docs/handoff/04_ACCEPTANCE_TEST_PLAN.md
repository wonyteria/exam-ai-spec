...
| AT-051 / REQ-16, REQ-14 / P0 / 설정·템플릿·기존 HWP 리브랜딩 | A/B 로고·metadata·학년/범위·미주정책·A4/B4, 진행job/과거revision; header표+그림, master title, odd/even/first header, 전용/header/master/print page number fixture | 설정 변경→새문서→기존문서 명시재적용→기존 HWP scan/후보확정→title교체·중앙watermark·쪽번호제거→재열기/이전판본 재출력 | immutable source/metadata/curriculum/template snapshot과version 유지. 새설정이 기존판본을 조용히 변경하지 않음. 선택 control 밖 본문·표·수식·도형·미주·구역·page count diff0. 모든 대상 page에 title/watermark 누락·중복0, 모든 page-number mechanism 제거, 다른 footer보존. timeout/crash 시 원본불변·final없음·Hwp leak0. tenant간누출·브랜드로원문덮기0 | I+E+W+H: source/output hash·control manifest·plan digest·masked render diff·actual reopen |
