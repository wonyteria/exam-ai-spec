...
- 안전장치: `extract_page`가 문항을 통째로 누락하면 폴백이 못 잡으므로,
  ZERO TYPO 게이트가 인쇄 번호(숫자 라벨) 연속성을 검사한다 —
  건너뜀은 `missing_object`+`missing_numbers` 목록, 중복은 `number_conflict`.
  리뷰 API/UI에 누락 번호가 표시된다.
